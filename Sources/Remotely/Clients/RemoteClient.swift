...
extension RemoteClient: DependencyKey {
    static let liveValue: Self = {
        let live = RemoteClientLive.shared
        return Self(
            events: { await live.events() },
            start: { await live.start() },
            stop: { await live.stop() },
            reconnect: { await live.reconnect() },
            refreshPermission: { await live.refreshPermission() },
            requestPermission: { await live.requestPermission() },
            setEnabled: { enabled in await live.setEnabled(enabled) },
            setSensitivity: { sensitivity in await live.setSensitivity(sensitivity) },
            setBinding: { binding, button in await live.setBinding(binding, for: button) },
            setAction: { action, button in await live.setAction(action, for: button) },
            setCombo: { combo, button in await live.setCombo(combo, for: button) },
            resetBinding: { button in await live.resetBinding(for: button) },
            resetAllBindings: { await live.resetAllBindings() },
            resetPreferences: { await live.resetPreferences() },
            clearLog: { await live.clearLog() }
        )
    }()
...
@MainActor
final class RemoteClientLive: @unchecked Sendable {
